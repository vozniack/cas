...
class PrivilegeMapper(private val privilegeService: PrivilegeService) : Mapper<Privilege, PrivilegeDto> {
    ...
    override fun mapToDto(entity: Privilege): PrivilegeDto =
        PrivilegeDto(entity.id, entity.name, entity.code, entity.description,
    ...
    override fun mapToEntity(dto: PrivilegeDto): Privilege =
        Privilege(dto.id, dto.name, dto.code, dto.description, Optional.ofNullable(dto.parentId)
            .map { parentId -> privilegeService.findById(parentId) }.orElse(null), null)
}